...
export class ConversionValidationStage implements ValidationStage {
  ...
  private async validateAddonStructure(
    ...
    if (structure.behaviorPack) {
      ...
      const missingDirs = requiredBehaviorDirs.filter(
        (dir) => !structure.behaviorPack.directories?.includes(dir)
      );
    ...
    if (structure.resourcePack) {
      ...
      const missingDirs = requiredResourceDirs.filter(
        (dir) => !structure.resourcePack.directories?.includes(dir)
      );
