...
export class SecurityValidationStage implements ValidationStage {
  ...
  private async validateMaliciousPatterns(
    ...
    const suspiciousPatterns = [
      { pattern: /Runtime\.getRuntime\(\)\.exec/, severity: ErrorSeverity.CRITICAL, description: 'Runtime execution detected' },
      { pattern: /ProcessBuilder/, severity: ErrorSeverity.CRITICAL, description: 'Process builder detected' },
      { pattern: /System\.exit/, severity: ErrorSeverity.ERROR, description: 'System exit call detected' },
      { pattern: /File\.delete/, severity: ErrorSeverity.WARNING, description: 'File deletion detected' },
      { pattern: /FileOutputStream/, severity: ErrorSeverity.WARNING, description: 'File output stream detected' },
      { pattern: /URLClassLoader/, severity: ErrorSeverity.WARNING, description: 'Dynamic class loading detected' },
      { pattern: /Reflection/, severity: ErrorSeverity.WARNING, description: 'Reflection usage detected' }
    ];
    ...
    for (const { pattern, severity, description } of suspiciousPatterns) {
      ...
      if (matches) {
        const error = createConversionError({
          code: createErrorCode(MODULE_ID, 'PATTERN', suspiciousPatterns.indexOf({ pattern, severity, description }) + 1),
          type: ErrorType.SECURITY,
          severity,
  ...
  private async validateAnalysisResults(
    ...
    if (analysisResults.registryNames) {
      const suspiciousNames = analysisResults.registryNames.filter((name: string) => 
    ...
    if (analysisResults.texturePaths) {
      const suspiciousPaths = analysisResults.texturePaths.filter((path: string) => 
        path.includes('..') || 
        path.startsWith('/') || 
        path.includes('\\')
      );
  ...
  private async validateConversionResults(
    ...
    if (conversionResults.outputFiles) {
      const suspiciousFiles = conversionResults.outputFiles.filter((file: any) => 
        file.path && (
          file.path.includes('..') ||
