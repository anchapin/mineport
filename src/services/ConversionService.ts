...
import { BedrockArchitect } from '../modules/conversion-agents/BedrockArchitect.js';
import { BlockItemGenerator } from '../modules/conversion-agents/BlockItemGenerator.js';
import { FeatureFlagService, MODPORTER_AI_FEATURES } from './FeatureFlagService.js';
...
export class ConversionService extends EventEmitter implements IConversionService {
  ...
  constructor(options: ConversionServiceOptions) {
    ...
    this.fileProcessor =
      options.fileProcessor || new FileProcessor({}, this.cacheService, this.performanceMonitor);
    this.javaAnalyzer =
      options.javaAnalyzer || new JavaAnalyzer(this.cacheService, this.performanceMonitor);
