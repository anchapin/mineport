...
export class EnhancedErrorCollector extends ErrorCollector implements IEnhancedErrorCollector {
  ...
  public getRecoverableErrors(): EnhancedConversionError[] {
    return Array.from(this.enhancedErrors.values())
      .filter(error => error.isRecoverable && !error.hasBeenRecovered);
  }
  ...
  public getErrorRateMetrics(timeWindow: number = 3600000): ErrorRateMetrics {
    ...
    const recentErrors = allErrors.filter(error => 
      error.timestamp >= startTime && error.timestamp <= now
    );
    ...
    recentErrors.forEach(error => {
      ...
      errorsBySeverity[error.severity.toString()] = (errorsBySeverity[error.severity.toString()] || 0) + 1;
    ...
    const firstHalfErrors = recentErrors.filter(error => error.timestamp >= startTime && error.timestamp < midPoint).length;
    const secondHalfErrors = recentErrors.filter(error => error.timestamp >= midPoint && error.timestamp <= now).length;
  ...
  public getSystemHealthStatus(): SystemHealthStatus {
    ...
    if (metrics.totalErrors >= 50) { // High error count threshold
      overall = 'critical';
      degradationLevel = Math.max(degradationLevel, 3);
    }
    
    // Check component health
    const failingComponents = Object.values(components).filter(c => c.status === 'failing').length;
    ...
    if (failingComponents > 0) {
      recommendations.push(`${failingComponents} components are failing - immediate attention required`);
    }
  ...
  private updateErrorRateHistory(): void {
    const now = new Date();
    const recentErrors = super.getErrors().filter(error => 
      error.timestamp >= new Date(now.getTime() - 60000) // Last minute
    );
    ...
    this.errorRateHistory = this.errorRateHistory.filter(entry => 
  ...
  private updateComponentHealth(): void {
    ...
    modules.forEach(module => {
      ...
      const recentErrors = moduleErrors.filter(error => 
        error.timestamp >= new Date(Date.now() - 300000) // Last 5 minutes
      );
      
      const errorRate = recentErrors.length / 5; // errors per minute
      const lastError = moduleErrors.length > 0 ? 
        moduleErrors[moduleErrors.length - 1].timestamp : undefined;
      ...
      const enhancedModuleErrors = Array.from(this.enhancedErrors.values())
        .filter(error => error.moduleOrigin === module);
      
      const recoveryAttempts = enhancedModuleErrors
        .reduce((sum, error) => sum + error.recoveryAttempts, 0);
      
      const fallbackActive = enhancedModuleErrors
        .some(error => error.hasBeenRecovered);
   ...
   * Categorize an error
   */
  private categorizeError(error: ConversionError): Omit<ErrorCategorization, 'frequency' | 'errors'> {
