...
export class ManualPostProcessingGuide {
   ...
   */
  public async generateGuide(input: PostProcessingGuideInput, outputDir: string): Promise<PostProcessingGuideOutput> {
  ...
  private generatePostProcessingSteps(input: PostProcessingGuideInput): PostProcessingStep[] {
    ...
    this.addErrorNoteSteps(input.conversionNotes.filter(note => note.severity === 'error'), steps);
    
    // Add steps for warning notes
    this.addWarningNoteSteps(input.conversionNotes.filter(note => note.severity === 'warning'), steps);
  ...
  * Adds post-processing steps for tier3 features
   ...
   */
  private createDimensionSimulationStep(feature: Feature, strategy: AppliedCompromiseStrategy): PostProcessingStep {
    return {
...
simulator.setSkyProperties({
  ...
  starBrightness: 0.8
      ...
      affectedFiles: [
        `scripts/dimension/${feature.name.toLowerCase().replace(/\s+/g, '_')}.js`
      ],
      relatedFeature: feature.id
    };
  }  /**
   ...
   * @returns A post-processing step
   */
  private createRenderingStubStep(feature: Feature, strategy: AppliedCompromiseStrategy): PostProcessingStep {
    return {
...
function replaceCustomRendering(entity) {
  ...
  world.spawnParticle("minecraft:basic_flame_particle", 
    ...
    { x: 0, y: 0.1, z: 0 });
      ...
      affectedFiles: [
        `scripts/rendering/${feature.name.toLowerCase().replace(/\s+/g, '_')}.js`
      ],
      relatedFeature: feature.id
    };
  }
   ...
   * @returns A post-processing step
   */
  private createUIFlowMappingStep(feature: Feature, strategy: AppliedCompromiseStrategy): PostProcessingStep {
    return {
      id: `ui-${feature.id}`,
...
form.setSize({ width: 0.8, height: 0.6 }); // 80% width, 60% height`,
      affectedFiles: [
        `scripts/ui/${feature.name.toLowerCase().replace(/\s+/g, '_')}.js`
      ],
      relatedFeature: feature.id
    };
  }  /**
...
/**
   ...
   */
  private getCodeSnippetForError(
    note: { type: string; message: string; sourceFile?: string; sourceLine?: number }
  ): string {
...
// TODO: Implement the fix based on the error message`;
   ...
   * @returns A code snippet
   */
  private getCodeSnippetForWarning(
    note: { type: string; message: string; sourceFile?: string; sourceLine?: number }
  ): string {
