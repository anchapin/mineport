...
export class LicenseEmbedder {
    ...
    try {
      ...
      for (const file of manifestFiles) {
        try {
          ...
          if (!manifest.metadata || !manifest.metadata.license || manifest.metadata.license !== licenseInfo.type) {
            errors.push(`Manifest file ${file} is missing proper license information`);
          }
        } catch (error) {
          errors.push(`Failed to check license information in ${file}: ${(error as Error).message}`);
        }
      }
  ...
  private async embedLicenseInManifest(
    ...
    try {
      ...
      throw new Error(`Failed to embed license in manifest ${filePath}: ${(error as Error).message}`);
    }
  }
