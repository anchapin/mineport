 ...
 */
export async function runBenchmark(name: string, fn: () => Promise<any>): Promise<BenchmarkResult> {
 ...
 */
export function saveBenchmarkResults(result: BenchmarkSuiteResult, outputPath: string): void {
 ...
 */
export function updateBenchmarkHistory(result: BenchmarkSuiteResult, historyPath: string): void {
  // Create directory if it doesn't exist
...
export function detectRegressions(
  ...
  for (const [name, data] of Object.entries(history.benchmarks)) {
    ...
    const baselineDuration =
      previousResults.reduce((sum, result) => sum + result.duration, 0) / previousResults.length;
 ...
 */
export function generateBenchmarkReport(historyPath: string, outputPath: string): void {
  // Load history
  if (!fs.existsSync(historyPath)) {
  ...
  for (const [name, data] of Object.entries(history.benchmarks)) {
    ...
    if (sortedResults.length >= 2) {
      ...
      const durationChange =
        ((latestResult.duration - firstResult.duration) / firstResult.duration) * 100;
      const memoryChange =
        ((latestResult.memoryUsage.heapUsed - firstResult.memoryUsage.heapUsed) /
          firstResult.memoryUsage.heapUsed) *
        100;
