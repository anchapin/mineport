...
import { 
  ...
  JavaSoundFile, 
  BedrockSoundFile
} from '../../../../src/modules/assets/SoundProcessor';
...
describe('SoundProcessor', () => {
  ...
  describe('convertSounds', () => {
    it('should convert all sounds successfully', async () => {
      ...
      expect(result.convertedSounds[2].path).toContain('sounds/entity/examplemod_example_entity.wav');
  ...
  describe('mapSoundPath', () => {
    it('should map Java sound paths to Bedrock paths correctly', () => {
      ...
      const blockPath = mapSoundPath.call(processor, 'assets/examplemod/sounds/block/example_block.ogg');
      expect(blockPath).toBe('sounds/block/examplemod_example_block.ogg');
      
      // Test item sound path mapping
      const itemPath = mapSoundPath.call(processor, 'assets/examplemod/sounds/item/example_item.mp3');
      expect(itemPath).toBe('sounds/item/examplemod_example_item.mp3');
      
      // Test entity sound path mapping
      const entityPath = mapSoundPath.call(processor, 'assets/examplemod/sounds/entity/example_entity.wav');
  ...
  describe('extractSoundEvent', () => {
    it('should extract sound event names correctly', () => {
      ...
      const blockEvent = extractSoundEvent.call(processor, 'assets/examplemod/sounds/block/example_block.ogg');
      expect(blockEvent).toBe('examplemod:block.example_block');
      
      // Test item sound event extraction
      const itemEvent = extractSoundEvent.call(processor, 'assets/examplemod/sounds/item/example_item.mp3');
      expect(itemEvent).toBe('examplemod:item.example_item');
      
      // Test entity sound event extraction
      const entityEvent = extractSoundEvent.call(processor, 'assets/examplemod/sounds/entity/example_entity.wav');
      expect(entityEvent).toBe('examplemod:entity.example_entity');
      
      // Test nested path extraction
      const nestedEvent = extractSoundEvent.call(processor, 'assets/examplemod/sounds/entity/example/nested/sound.wav');
  ...
  describe('generateSoundsJson', () => {
    it('should generate a valid sounds.json structure', () => {
      ...
      expect(result.sound_definitions['examplemod:block.example_block'].sounds[0].name).toBe('block/examplemod_example_block');
      ...
      expect(result.sound_definitions['examplemod:item.example_item'].sounds[0].name).toBe('item/examplemod_example_item');
