...
import {
  ...
  PostProcessingGuideInput,
} from '../../../../src/modules/packaging/ManualPostProcessingGuide.js';
...
describe('ManualPostProcessingGuide', () => {
  ...
  it('should generate steps for tier3 features with compromise strategies', async () => {
    ...
    const dimensionStep = result.steps.find(
      (step) => step.title.includes('Dimension') && step.priority === 'high'
    );
  ...
  it('should generate steps for tier4 features', async () => {
    ...
    const unanalyzableStep = result.steps.find(
      (step) => step.title.includes('Manually Implement') && step.priority === 'critical'
    );
  ...
  it('should generate steps for error notes', async () => {
    ...
    const errorStep = result.steps.find(
      (step) => step.title.includes('Fix script Error') && step.priority === 'critical'
    );
  ...
  it('should generate steps for warning notes', async () => {
    ...
    const warningStep = result.steps.find(
      (step) => step.title.includes('Address texture Warning') && step.priority === 'medium'
    );
