...
describe('FeatureCompatibilityAnalyzer', () => {
  ...
  it('should analyze mod features and categorize them correctly', async () => {
    ...
    const tier1Features = result.features.filter(
      (f) => f.compatibilityTier === CompatibilityTier.FULLY_TRANSLATABLE
    );
    const tier2Features = result.features.filter(
      (f) => f.compatibilityTier === CompatibilityTier.APPROXIMATION_POSSIBLE
    );
    const tier3Features = result.features.filter(
      (f) => f.compatibilityTier === CompatibilityTier.NATIVELY_IMPOSSIBLE
    );
  ...
  it('should detect assets and categorize them correctly', async () => {
    ...
    expect(
      assetFeatures.every((f) => f.compatibilityTier === CompatibilityTier.FULLY_TRANSLATABLE)
    ).toBe(true);
  });
  ...
  it('should detect configuration files and categorize them correctly', async () => {
    ...
    expect(
      configFeatures.every((f) => f.compatibilityTier === CompatibilityTier.FULLY_TRANSLATABLE)
    ).toBe(true);
  });
  ...
  it('should analyze Java code for API usage', async () => {
    ...
    const mockApiFiles = {
      '/tmp/mod/src/main/java/com/example/testmod/api/VanillaApi.java':
        'package com.example.testmod.api;\n\nimport net.minecraft.world.item.Item;\nimport net.minecraft.world.level.block.Block;\n\npublic class VanillaApi {\n  public void useVanillaApi() {\n    Item item = new Item(new Item.Properties());\n    Block block = new Block(Block.Properties.of(Material.STONE));\n  }\n}',
      '/tmp/mod/src/main/java/com/example/testmod/api/ForgeApi.java':
        'package com.example.testmod.api;\n\nimport net.minecraftforge.common.MinecraftForge;\nimport net.minecraftforge.event.entity.player.PlayerEvent;\n\npublic class ForgeApi {\n  public void useForgeApi() {\n    MinecraftForge.EVENT_BUS.register(this);\n  }\n  \n  public void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {\n    // Handle event\n  }\n}',
      '/tmp/mod/src/main/java/com/example/testmod/api/MixedApi.java':
    ...
    expect(forgeApiFeature?.compatibilityTier).toBeGreaterThanOrEqual(
      CompatibilityTier.APPROXIMATION_POSSIBLE
    );
  });
