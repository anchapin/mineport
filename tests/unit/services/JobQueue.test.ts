...
import { JobQueue, Job } from '../../../src/services/JobQueue';
...
describe('JobQueue', () => {
  ...
  it('should emit events when job status changes', () => {
    ...
    expect(addedListener).toHaveBeenCalledWith(expect.objectContaining({
      id: job.id,
      type: 'test',
    }));
    
    expect(processListener).toHaveBeenCalledWith(expect.objectContaining({
      id: job.id,
      status: 'processing',
    }));
    ...
    expect(completedListener).toHaveBeenCalledWith(expect.objectContaining({
      id: job.id,
      status: 'completed',
