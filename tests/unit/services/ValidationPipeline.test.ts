...
} from '../../../src/services/ValidationPipeline';
...
class MockValidationStage implements ValidationStage {
  ...
  async validate(input: ValidationInput, config?: Record<string, any>): Promise<ValidationStageResult> {
    if (this.shouldThrow) {
...
describe('ValidationPipeline', () => {
  ...
  describe('stage management', () => {
    ...
    it('should prevent duplicate stage names', () => {
      ...
      expect(() => pipeline.addStage(stage2)).toThrow('Validation stage \'duplicate\' already exists');
    });
  ...
  describe('error collection', () => {
    ...
    it('should clear error collector before validation', async () => {
      ...
      errorCollector.addError(createConversionError({
