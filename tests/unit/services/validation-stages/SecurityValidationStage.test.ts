...
describe('SecurityValidationStage', () => {
  ...
  describe('analysis results validation', () => {
    it('should detect suspicious registry names', async () => {
      ...
      expect(result.warnings[0].details?.suspiciousNames).toEqual(['hack_tool', 'exploit_item', 'backdoor_entity']);
    });
    ...
    it('should detect suspicious texture paths', async () => {
      const input: ValidationInput = {
        ...
        fileContent: Buffer.from('content'),
        analysisResults: {
          texturePaths: ['textures/block/stone.png', '../../../system/hack.png', '/root/exploit.png']
        }
      };
      ...
      expect(result.errors[0].details?.suspiciousPaths).toEqual(['../../../system/hack.png', '/root/exploit.png']);
    });
  ...
  describe('conversion results validation', () => {
    it('should detect suspicious output file paths', async () => {
      ...
      expect(result.errors[0].details?.suspiciousFiles).toEqual(['../../../system/malicious.exe', '/admin/backdoor.js']);
    });
