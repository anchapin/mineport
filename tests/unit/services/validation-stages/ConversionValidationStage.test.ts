...
import { ConversionValidationStage } from '../../../../src/services/validation-stages/ConversionValidationStage';
...
describe('ConversionValidationStage', () => {
  ...
  describe('addon structure validation', () => {
    ...
    it('should warn about missing expected directories', async () => {
      ...
      expect(result.warnings.some(w => w.message.includes('Behavior pack missing expected directories'))).toBe(true);
      expect(result.warnings.some(w => w.message.includes('Resource pack missing expected directories'))).toBe(true);
    });
        ...
        conversionResults: {
          ...
          manifests: {
            ...
            resourcePack: {
              ...
              modules: [
                { type: 'resources', uuid: '66666666-7777-8888-9999-aaaaaaaaaaaa', version: [1, 0, 0] }
              ]
            }
          }
        }
  ...
  describe('manifest files validation', () => {
      ...
      const input: ValidationInput = {
        ...
        conversionResults: {
          ...
          manifests: {
            resourcePack: {
              format_version: '1.16.0',
              header: { name: 'Test', description: 'Test', uuid: '12345678-1234-1234-1234-123456789abc', version: [1, 0, 0] },
              modules: []
            }
          }
        }
    ...
    it('should warn about invalid version format', async () => {
      const input: ValidationInput = {
        ...
        conversionResults: {
          ...
          manifests: {
            ...
            resourcePack: {
              format_version: '1.16.0',
              header: { name: 'Test', description: 'Test', uuid: '11111111-2222-3333-4444-555555555555', version: [1, 0, 0] },
              modules: []
            }
          }
        }
      ...
      expect(result.passed).toBe(true);
      expect(result.warnings).toHaveLength(1);
      expect(result.warnings[0].message).toContain('version should be [major, minor, patch] format');
    });
  });
    ...
    it('should warn when no converted assets found', async () => {
      const input: ValidationInput = {
        ...
        conversionResults: {
          ...
          manifests: {
            behaviorPack: { format_version: '1.16.0', header: { name: 'Test', description: 'Test', uuid: '12345678-1234-1234-1234-123456789abc', version: [1, 0, 0] }, modules: [] },
            resourcePack: { format_version: '1.16.0', header: { name: 'Test', description: 'Test', uuid: '11111111-2222-3333-4444-555555555555', version: [1, 0, 0] }, modules: [] }
          }
        }
      };
      ...
      expect(result.passed).toBe(true);
      expect(result.warnings.some(w => w.message.includes('No converted assets found'))).toBe(true);
    });
    ...
    it('should warn when texture is not converted to PNG', async () => {
      const input: ValidationInput = {
        ...
        conversionResults: {
          ...
          convertedAssets: {
            textures: [
              { originalPath: 'stone.jpg', convertedPath: 'textures/blocks/stone.jpg' }
            ]
          }
        }
      };
    ...
    it('should warn when model is not converted to Bedrock format', async () => {
      const input: ValidationInput = {
        ...
        conversionResults: {
          ...
          convertedAssets: {
            models: [
              { originalPath: 'block.json', convertedPath: 'models/block.json' }
            ]
          }
        }
      };
    ...
    it('should warn when sound is not converted to supported format', async () => {
      const input: ValidationInput = {
        ...
        conversionResults: {
          ...
          convertedAssets: {
            sounds: [
              { originalPath: 'sound.mp3', convertedPath: 'sounds/sound.mp3' }
            ]
          }
        }
      };
  ...
  describe('behavior pack content validation', () => {
    it('should warn when no behavior pack content found', async () => {
      ...
      expect(result.passed).toBe(true);
      expect(result.warnings.some(w => w.message.includes('No behavior pack content found'))).toBe(true);
    });
    ...
    it('should warn about potential script syntax errors', async () => {
      const input: ValidationInput = {
        ...
        conversionResults: {
          ...
          behaviorPack: {
            scripts: [
              { path: 'broken.js', content: 'function test() { console.log("missing closing brace");' }
            ]
          }
        }
      };
    ...
    it('should fail when block definition is missing required fields', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.errors).toHaveLength(1);
      expect(result.errors[0].message).toContain('Block 1 missing required identifier or definition');
    });
  });
  ...
  describe('conversion completeness validation', () => {
    it('should warn about low asset conversion count', async () => {
      ...
      expect(result.passed).toBe(true);
      expect(result.warnings.some(w => w.message.includes('Low asset conversion count'))).toBe(true);
    });
    ...
    it('should warn about low script conversion count', async () => {
      ...
      expect(result.passed).toBe(true);
      expect(result.warnings.some(w => w.message.includes('Low script conversion count'))).toBe(true);
    });
  });
  ...
  describe('output file integrity validation', () => {
    it('should warn when no output files list found', async () => {
      ...
      expect(result.passed).toBe(true);
      expect(result.warnings.some(w => w.message.includes('No output files list found'))).toBe(true);
    });
