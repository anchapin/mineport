...
import { AnalysisValidationStage } from '../../../../src/services/validation-stages/AnalysisValidationStage.js';
...
describe('AnalysisValidationStage', () => {
  ...
  describe('registry names validation', () => {
    ...
    it('should warn when registry names array is empty', async () => {
      ...
      expect(result.passed).toBe(true);
      expect(result.warnings.some((w) => w.message.includes('No registry names extracted'))).toBe(
        true
      );
    });
  ...
  describe('manifest info validation', () => {
    it('should warn when no manifest info found', async () => {
      ...
      expect(result.passed).toBe(true);
      expect(result.warnings.some((w) => w.message.includes('No manifest information found'))).toBe(
        true
      );
    });
      ...
      const input: ValidationInput = {
        ...
        analysisResults: {
          ...
          manifestInfo: {
            ...
            author: 'Test Author',
            dependencies: [{ modId: 'forge', version: '36.2.0', required: true }],
          },
        },
      };
  ...
  describe('extraction completeness validation', () => {
    ...
    it("should warn when registry names don't match mod ID", async () => {
      ...
      expect(result.passed).toBe(true);
      expect(
        result.warnings.some((w) => w.message.includes('No registry names match the mod ID'))
      ).toBe(true);
    });
    ...
    it('should pass when registry names match mod ID', async () => {
      ...
      expect(result.passed).toBe(true);
      expect(
        result.warnings.filter((w) => w.message.includes('No registry names match')).length
      ).toBe(0);
    });
  });
