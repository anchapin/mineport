  ...
  describe('Interface Compliance', () => {
    it('should validate that all modules implement required interfaces', async () => {
      ...
      for (const moduleFile of moduleFiles) {
        ...
        const hasProcessMethod =
          /(?:process|convert|translate|analyze|generate|validate)\s*\([^)]*\)\s*:\s*Promise</.test(
            content
          );
        if (!hasProcessMethod) {
    ...
    it('should validate that all modules use consistent type imports', async () => {
      ...
      for (const moduleFile of moduleFiles) {
        ...
        if (usesErrors && !importsErrorTypes) {
          typeImportViolations.push(
            `${moduleFile}: Uses errors but missing ConversionError import`
          );
        }
      }
    ...
    it('should validate that all type definitions are properly structured', async () => {
      ...
      for (const typeFile of typeFiles) {
        ...
        if (camelCaseViolations.length > 0) {
          typeStructureViolations.push(
            `${typeFile}: Non-camelCase properties: ${camelCaseViolations.join(', ')}`
          );
        }
      }
  ...
  describe('Error Handling Consistency', () => {
    it('should validate that all modules use consistent error format', async () => {
      ...
      for (const moduleFile of moduleFiles) {
        ...
        const methods =
          content.match(
            /(?:async\s+)?(?:process|convert|translate|analyze|generate|validate)\s*\([^)]*\)/g
          ) || [];
  ...
  describe('Naming Convention Compliance', () => {
      ...
      for (const moduleFile of moduleFiles) {
        ...
        if (classMatch && classMatch[1] !== fileName) {
          namingViolations.push(
            `${moduleFile}: Class name '${classMatch[1]}' should match file name '${fileName}'`
          );
        }
      }
        ...
        for (const interfaceDecl of interfaces) {
          ...
          if (interfaceName) {
            ...
            if (!/^[A-Z][a-zA-Z0-9]*$/.test(interfaceName)) {
              interfaceNamingViolations.push(
                `${typeFile}: Interface '${interfaceName}' should be PascalCase`
              );
            }
          }
        }
        ...
        for (const typeDecl of types) {
          ...
          if (typeName) {
            ...
            if (!/^[A-Z][a-zA-Z0-9]*$/.test(typeName)) {
              interfaceNamingViolations.push(
                `${typeFile}: Type '${typeName}' should be PascalCase`
              );
            }
          }
        }
        ...
        for (const enumDecl of enums) {
          ...
          if (enumName) {
            ...
            if (!/^[A-Z][a-zA-Z0-9]*$/.test(enumName)) {
              interfaceNamingViolations.push(
                `${typeFile}: Enum '${enumName}' should be PascalCase`
              );
            }
          }
        }
    ...
    it('should validate that method names follow conventions', async () => {
      ...
      for (const moduleFile of moduleFiles) {
        ...
        const methods =
          content.match(/(?:public|private|protected)?\s*(?:async\s+)?(\w+)\s*\([^)]*\)/g) || [];
        ...
        for (const method of methods) {
          ...
          if (methodName && methodName !== 'constructor') {
            ...
            if (!/^[a-z][a-zA-Z0-9]*$/.test(methodName)) {
              methodNamingViolations.push(
                `${moduleFile}: Method '${methodName}' should be camelCase`
              );
            }
          }
        }
        ...
        for (const constant of constants) {
          ...
          if (constantName) {
            ...
            if (!/^[A-Z][A-Z0-9_]*$/.test(constantName)) {
              constantNamingViolations.push(
                `${moduleFile}: Constant '${constantName}' should be SCREAMING_SNAKE_CASE`
              );
            }
          }
        }
  ...
  describe('Module Structure Consistency', () => {
    ...
    it('should validate that all modules have proper dependency imports', async () => {
      ...
      for (const moduleFile of moduleFiles) {
        ...
        if (relativeImports.length > 2) {
          // Allow some relative imports
          dependencyViolations.push(
            `${moduleFile}: Too many relative imports, consider using absolute imports`
          );
        }

        // Check for proper service imports
        const usesServices = /(?:ErrorCollector|ConfigurationService|ConversionPipeline)/.test(
          content
        );
