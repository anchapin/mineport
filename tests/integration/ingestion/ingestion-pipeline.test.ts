...
import { LicenseParser } from '../../../src/modules/ingestion/LicenseParser.js';
...
describe('Ingestion Pipeline Integration', () => {
  ...
  it('should process a mod through the complete ingestion pipeline', async () => {
    ...
    const extractPath = await modValidator.extractMod(
      fs.readFileSync(modFile),
      validationResult.modId
    );
