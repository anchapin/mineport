...
class AuditAnalyzer {
  ...
  findFailurePatterns(entries) {
    const failedEntries = entries.filter((e) => e.status === 'failed' || e.status === 'error');
    ...
    failedEntries.forEach((entry) => {
      patterns.by_operation[entry.operation] = (patterns.by_operation[entry.operation] || 0) + 1;
      patterns.by_actor[entry.actor] = (patterns.by_actor[entry.actor] || 0) + 1;
  ...
  findDeploymentPatterns(entries) {
    const deploymentEntries = entries.filter((e) => e.operation === 'deployment');
  ...
  detectUnusualActors(entries) {
    ...
    const average =
      Object.values(actorCounts).reduce((a, b) => a + b, 0) / Object.keys(actorCounts).length;
  ...
  detectSecurityAnomalies(entries) {
    const securityEntries = entries.filter(
      (e) => e.operation.includes('security') || e.operation.includes('compliance')
    );

    const anomalies = [];

    // Check for failed security scans
    const failedScans = securityEntries.filter(
  ...
  checkAccessControls(entries) {
    ...
    const privilegedOperations = entries.filter(
      (e) => e.operation === 'deployment' || e.operation.includes('security')
    );
  ...
  checkChangeManagement(entries) {
    ...
    const approvedDeployments = deployments.filter(
      (e) => e.metadata?.approved === true || e.metadata?.approval_required === false
    );
    ...
    return {
      ...
      approval_rate:
        deployments.length > 0 ? (approvedDeployments.length / deployments.length) * 100 : 100,
    };
  }
