...
class FeatureFlagManager {
    ...
    disableFeature(featureName) {
        ...
        if (dependentFeatures.length > 0) {
            throw new Error(`Cannot disable '${featureName}' - required by: ${dependentFeatures.join(', ')}`);
        }
    ...
    advanceToNextPhase() {
        ...
        if (strategy.currentPhase < strategy.phases.length - 1) {
            ...
            this.saveFlags();
            console.log(`📈 Advanced to phase ${strategy.currentPhase + 1}: ${strategy.phases[strategy.currentPhase].name}`);
            return true;
        } else {
    ...
    try {
        switch (command) {
            ...
            default:
                ...
                console.log('  enable <feature> [%]     - Enable a feature with optional rollout percentage');
